...
for i in range(10, 18):
    if i == 9:
        continue
    if i < 10:
        df = df1[(df1.case_num == "case0" + str(i))]
    else:
        fig, axes = plt.subplots(1, 2)
        plt.subplots_adjust(wspace=0.4)
        df = df1[(df1.case_num == "case" + str(i))]
    ...
    if i < 5:
        bplot1 = sns.boxplot(x = df_sgemm.exp, y = df_sgemm.pwr, palette="Set2")
        bplot1 = sns.swarmplot(x = df_sgemm.exp, y = df_sgemm.pwr, hue = df_sgemm.device)
    if i > 4 and i < 10:
        bplot1 = sns.boxplot(x = df_resnet.exp, y = df_resnet.pwr, palette="Set2")
        bplot1 = sns.swarmplot(x = df_resnet.exp, y = df_resnet.pwr, hue = df_resnet.device)
    if i >= 10:
    ...
    fig.savefig(str(i) + "_power_scatterplot.png", dpi=300)
